...
"""Single entry point for the `esg_kg` stages — run from the REPO ROOT.
    ...
    python src/run.py --list                       # every stage + migration status
    python src/run.py quality --label baseline     # run one stage
    python src/run.py 00 --label baseline          # ...by its old step id
    python src/run.py quality -- --help            # stage's own --help
...
package one level down in `src/`, so `python -m esg_kg.report.quality`
only resolves with `src/` on the path. The three ways to get it there are
...
`python -m esg_kg.report.quality` from inside `src/` remains equally
...
def print_list() -> None:
    ...
    print("\n  ready   -> python src/run.py <name> [args]")
    if skipped:
    ...
    if BLOCKS:
        ...
        print("\n  block   -> python src/run.py <block name> [args]")
...
def main(argv: list) -> int:
    ...
    if hit is None:
        ...
        print("run `python src/run.py --list` for the stage names", file=sys.stderr)
        return 2
