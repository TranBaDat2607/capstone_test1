# Pipeline của đợt refactor — cây phẳng `src/stepNN_*.py` → `src/esg_kg/`

> **Lưu ý khi đọc (2026-07-30):** thư mục chứa file này từng tên `src_module/`, nay là
> `src/`. Bên dưới, `src/` trơ trọi = **cây phẳng cũ** (mỗi stage một `stepNN_*.py`, đã
> xoá 2026-07-29), không phải thư mục hiện tại.
...
là bản vẽ của cùng dữ liệu đó. `python src/run.py --list` luôn nói thật về tiến độ.
...
- **`09` là stage ĐỌC Neo4j ĐẦU TIÊN dời** — khác hẳn `06`/`08` vốn chỉ ghi. `load_from_
  ...
  to_neo4j.py` sang `src/run.py neo4j_sync` — đúng khuôn đổi 1 dòng thông báo mà
...
> hoàn tất, 15/15**.
> Nghi ngờ thì hỏi `python src/run.py --list`, đừng đọc màu.

| Nhãn trong ô | Màu | Nghĩa |
|---|---|---|
| `✅ ĐÃ DỜI` | 🟩 xanh **đặc**, chữ trắng | đã dời sang `esg_kg` — chạy bằng `python src/run.py <tên>` |
...
1. **Kernel đã hết đường chặn.** Sau `core/llm.py` (2026-07-27) có 8/11 stage chưa dời đủ
   ...
   - **`09` — mở khoá bởi `07` (qua §2 điểm 1 trên), stage ĐỌC Neo4j đầu tiên** → **ĐÃ DỜI
     ...
     `src/run.py neo4j_sync`, đúng khuôn đổi 1 dòng thông báo mà `04`/`06`/`08` đã
...
```bash
python src/run.py build_validated --dry-run   # 03+03b+03c offline, không ghi gì
python src/run.py build_validated             # ghi all_validated_triples.json MỘT lần
```
...
```bash
python src/run.py build_resolved --dry-run   # 05+05b+05c offline, không ghi gì
python src/run.py build_resolved             # ghi resolved_graph.json MỘT lần
```
