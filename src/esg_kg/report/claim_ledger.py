...
Run from the repo root:
    python src/run.py claim_ledger                       # AAA ledger (contradicted + supported)
    python src/run.py claim_ledger --review-queue        # contradiction, no independent verification
    python src/run.py claim_ledger --assessment all --markdown
    python src/run.py claim_ledger --claim-id AAA_SC_001
    ...
    with driver.session(database=database) as s:
        ...
        if not claim_rows:
            logger.error(f"No claims with an assessment for ticker '{t}' in Neo4j. "
                         f"Run: python src/run.py neo4j_sync --ticker {t}")
            sys.exit(1)
...
def render_header_text(h: Dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"  (source: Neo4j advisory layer — run src/run.py neo4j_sync to refresh)",
        "=" * 88,
    ])
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(
        description="Step 7 / P5 — render the per-company claim ledger from the Neo4j advisory "
                    "layer (read-only; run src/run.py neo4j_sync first).")
