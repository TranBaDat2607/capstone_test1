...
Script: [`src/esg_kg/graph/fix_triples.py`](../src/esg_kg/graph/fix_triples.py)

This step takes the per-page temporal graphs produced by
[`step02_extract_triplet_from_jsonl.py`](../src/esg_kg/graph/extract_triples.py) and turns
...
running `python src/run.py fix_triples` covers all 13 annual reports at
...
[`step02_extract_triplet_from_jsonl.py`](../src/esg_kg/graph/extract_triples.py).
...
python src/run.py fix_triples

# Just see the counts, don't spend any tokens
python src/run.py fix_triples --dry-run

# Smaller batches if a single batch is timing out
python src/run.py fix_triples --batch-size 10

# Paid Tier 1: crank the throttle
python src/run.py fix_triples --rate-limit 1000
```
