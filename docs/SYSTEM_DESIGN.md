...
`src/resolve_entities.py` already lists the claim/conduct classes in `OBSERVATION_CLASSES`:
...
flowchart TD
    ...
    R4 --> S1["Step 1: extract_kpi_from_jsonl"]
    N4 --> S1
    S1 --> S2["Step 2: extract_triplet_from_jsonl<br/>NEW --source report / news"]
    R4 --> S2
    N4 --> S2
    S2 --> S3["Step 3: fix_invalid_triplets"]
    S3 --> S4["Step 4: resolve_entities<br/>issuer anchor = one AAA node"]
    S4 --> S5["Step 5: load_graph_to_neo4j"]
    S5 --> S6["NEW Step 6: crosscheck_claims_vs_conduct<br/>retrieve + LLM adjudicate + structural + KPI"]
...
| 1 | `src/extract_kpi_from_jsonl.py` | none | Typed `KPIObservation`s from reports (and optionally news, for observed numbers) |
| 2 | `src/extract_triplet_from_jsonl.py` | **`--source` mode + news prompt** | Page → triples; stamps `source_type`; news mode biases to conduct classes (§6.1) |
| 3 | `src/fix_invalid_triplets.py` | none | Schema-validate / repair triples from **both** channels |
...
New script: **`src/crosscheck_claims_vs_conduct.py`**. It runs *after* the graph is built and
...
4. **Embedding rank** — remaining candidates are ranked by `gemini-embedding-001` cosine
   ...
   `src/resolve_entities.py`, Stage B.2). Keep the top-`k` above a threshold.
...
`src/report_claim_ledger.py` renders, for a company, every `SustainabilityClaim` with its
...
| `src/extract_triplet_from_jsonl.py` | Step 2 (+ `--source news` mode, news prompt, `source_type` stamping, `--dry-run`) | **done (P2)** |
