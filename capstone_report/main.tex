...
ESG-classified (873,756 sentences); alongside it, 4,582 news documents were
retrieved --- 3,797 of them distinct articles once cross-ticker duplicates are
removed --- from 716 Vietnamese online outlets. Both stand as a resource in
their own right. The framework itself was then instantiated end to end on
five issuers, yielding a resolved graph of 10,634 nodes and 14,744 edges with
100\% schema compliance and 97.78\% round-trip grounding of extracted values
to their cited page. Cross-checking those issuers' 464 claims returns 448
(96.55\%) unverified --- the direct consequence of a conduct channel roughly
sixteen times smaller than the claim side, which the design reports as
abstention rather than forcing a verdict. Only 16 of the 464 claims (3.4\%)
receive any supported/contradicted verdict at all, and every one of them
belongs to a single issuer, ACG; the graph-native indicator-axis retrieval
path contributed none of the 401 adjudicated pairs, so retrieval in the
system as evaluated here is Vietnamese token overlap with a temporal window,
not yet the graph-structured join the design also specifies
(\S\ref{sec:discussion-aggregate-base}). Of the 43 (claim, evidence) pairs
the stage currently cites, every pair carries blind labels from two external
domain experts, who confirm 27 (62.8\%, [47.9, 75.6]) and 28 (65.1\%,
[50.2, 77.6]) of them respectively and disagree with one another on only one
pair (\S\ref{sec:experiment-expert}) --- a result concentrated in the same
issuer, ACG (42 of the 43 pairs), rather than general across the five.
Scaling the graph beyond five issuers is engineering already specified by
the pipeline, not an open research question.
...
\begin{enumerate}
    ...
    \item A label-free evaluation instrument --- temporal-invariant linting, schema-compliance checking and a retrieval negative control, none of which need a ground-truth label to run --- demonstrated on a controlled before/after: the negative control's same-company evidence-attribution check (\S\ref{sec:experiment-internal}) moved from 28.76\% (failing) to 100\% (passing, zero cross-feed citations) once the adjudication-cache defect of \S\ref{sec:crosscheck} was fixed, showing the instrument can detect a real regression and its repair without any label existing for the underlying task.
...
that is not the authoritative publisher of the filings --- a limitation
returned to in \S\ref{sec:discussion-threats}.
...
an exact page number per sentence, not table reconstruction, and that
trade-off is a design choice argued for here rather than quantified by a
head-to-head ablation against the layout-aware parser, which this thesis
does not run.
...
Table~\ref{tab:conduct-channel} is a snapshot of one crawl, taken when this
chapter's exploratory analysis was run, and is internally consistent on its
own terms ($4{,}537\times36.2\approx164{,}036$). The corpus continued to grow
after that snapshot: by the time Chapter~\ref{ch:experiment} measures the
same channel, the news side carries 174,256 sentences on disk, not 164,036,
and a raw document count of 4,537 counts several re-crawled URLs under more
than one ticker --- 785 of 4,582 crawled documents (17.1\%) share a URL with
another ticker's feed. Retrieval-independence figures cited elsewhere in
this thesis (the Abstract, Contribution 2) therefore use the
de-duplicated, later count --- 3{,}797 distinct articles (4{,}582 documents
crawled) from 716 outlets --- rather than this table's 4,537/662, which is
reported here as-measured rather than restated to match.

\textbf{Independence is structurally sound.} Figure~\ref{fig:conduct-channel}(a) shows the twelve most frequent domains from this snapshot: the 4,537 articles come from 662
...
such in \S\ref{sec:discussion-threats}.
...
\textbf{The evaluation base is narrow, on both of its strongest legs.} \S\ref{sec:discussion-aggregate-base} above states this precisely: 16 of 464 claims verdicted, 42 of 43 blind-labelled pairs belonging to one issuer, and a graph-native retrieval path that fired zero times out of 401 adjudicated pairs. No claim in this thesis about precision or agreement should be read as established across the five issuers evaluated, still less across the sector the underlying corpus covers.
...
\textbf{The graph-native indicator-axis retrieval path has not yet fired.} Restated from \S\ref{sec:discussion-aggregate-base}: zero of 401 adjudicated pairs, across all five issuers, were retrieved through the \texttt{alignsWithIndicator}\allowbreak/\allowbreak\texttt{measuredUnder} structural join rather than lexical token overlap, for the structural reason given there (financial-unit KPIs on the conduct side are rejected by canonicalisation, not mapped). The schema and the join both exist and are exercised in the wrong direction --- alignment coverage, not retrieval --- so this is a real capability gap in the evaluated system, not a documentation gap.
...
\textbf{The resolved graph accumulates indicator-axis edges across runs rather than being rebuilt identically.} \S\ref{sec:experiment-yield} already reconciles the resulting 649-versus-718 figures in Table~\ref{tab:resolve-yield}; the underlying cause --- an append-only pass that protects previously-paid cross-check dossiers from being invalidated by a later re-run, at the cost of the graph not being exactly reproducible from a clean rebuild --- is a deliberate trade-off (\S\ref{sec:entity-resolution}), not an oversight, but it does mean a reader comparing two runs of this pipeline byte-for-byte should not expect an exact match on this edge population.
