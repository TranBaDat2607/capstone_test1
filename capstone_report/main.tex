...
\textbf{The graph-native indicator-axis retrieval path has not yet fired.} Restated from \S\ref{sec:discussion-aggregate-base}: zero of 401 adjudicated pairs, across all five issuers, were retrieved through the \texttt{alignsWithIndicator}/\texttt{measuredUnder} structural join rather than lexical token overlap, for the structural reason given there (financial-unit KPIs on the conduct side are rejected by canonicalisation, not mapped). The schema and the join both exist and are exercised in the wrong direction --- alignment coverage, not retrieval --- so this is a real capability gap in the evaluated system, not a documentation gap.
