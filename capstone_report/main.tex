...
the 24 (claim, evidence) pairs the stage cited at annotation time, 15 carry
blind labels from two external domain experts, who confirm 13 of them
(86.7\%, [62.1, 96.3]) and assign identical labels on all 15; those 15
concern one issuer, so the figure describes precision on a narrow and
deliberately conservative output rather than on the system at large, and a
later cache-key fix has since moved the stage's cited output to 43 pairs,
not yet re-annotated (\S\ref{sec:experiment-expert}). Scaling the graph
...
At the time this annotation was designed and run, the stage cited $226$ pairs across the five issuers ($99$ \textsf{supports} + $127$ \textsf{contradicts}). $200$ of those were sampled for blind labelling, stratified by issuer $\times$ asserted relation with seed $42$. A later retrieval-side tightening --- issuer-scoped conduct pools, Vietnamese-aware tokenization, a stricter topic-overlap gate --- cut the stage's cited output from $226$ pairs to $24$; $15$ of those $24$ happen to already carry a blind label from the $200$-pair round, and \textbf{those $15$, drawn from the $226$-pair population, are the narrow population this section's headline figures measure} --- not the stage's full cited output at any point in time. The remaining $9$ of the $24$ are unlabelled, and no figure in this thesis is claimed for them.

\textbf{The population has moved again since.} On 2026-08-13 an adjudicator cache-key defect was fixed (a stale cached verdict was surviving a prompt change it should have invalidated) and the stage was re-run against all five issuers; it now cites $43$ pairs --- not $24$ and not $226$. That $43$-pair output has \textbf{not been re-annotated}: every figure in this section, including the $86.7\%$ headline below, describes the $15$-pair (or, where stated, $200$-pair) population as it stood before this fix. None of it should be read as a measurement of the system's current cited output.
...
Let $S$ be the set of cited pairs carrying expert labels, $|S|=15$. Precision for expert $k$ is the fraction of them whose asserted relation the expert independently confirms:
\[
...
\begin{table}[htbp]
  ...
  \caption[Adjudicator precision against expert annotation]{Adjudicator
    precision against each expert's blind labels, over the $15$ cited pairs
  ...
  \begin{tabularx}{\linewidth}{|L{1.16}|L{0.92}|L{0.92}|}
    ...
    Precision, labelled cited pairs & \textbf{86.7\%} (13/15) \newline [62.1, 96.3] & \textbf{86.7\%} (13/15) \newline [62.1, 96.3] \\ \hline
    -- pairs asserted \textsf{supports} & 83.3\% (10/12) \newline [55.2, 95.3] & 83.3\% (10/12) \newline [55.2, 95.3] \\ \hline
    -- pairs asserted \textsf{contradicts} & 100\% (3/3) \newline [43.8, 100] & 100\% (3/3) \newline [43.8, 100] \\ \hline
  \end{tabularx}
\end{table}

Table~\ref{tab:expert-precision} gives the result. Both experts confirm thirteen of the fifteen, and they confirm the same thirteen. Both rejected pairs sit on the \textsf{supports} branch; neither expert rejected an asserted contradiction, which is the output that matters most, since a contradiction is an implicit allegation against a named real company. Table~\ref{tab:expert-confusion} gives the full distribution, and the shape of the two failures is worth naming: both were judged \textsf{irrelevant} rather than reversed, meaning the system was not disagreeing with the experts about the direction of the evidence so much as citing something with no bearing on the claim.

\begin{table}[htbp]
  \centering \small
  \caption[Where the adjudicator's citations land]{System-asserted relation
    (rows) against expert label (columns), over the $15$ labelled pairs. Both
    experts produced this same table.}
  \label{tab:expert-confusion}
  \begin{tabularx}{\linewidth}{|L{1.90}|C{0.70}|C{0.70}|C{0.70}|}
    \hline
    \rowcolor{hdrbg}
    \multicolumn{1}{|c|}{\textbf{System asserts}} &
    \multicolumn{1}{c|}{\textsf{sup.}} &
    \multicolumn{1}{c|}{\textsf{con.}} &
    \multicolumn{1}{c|}{\textsf{irr.}} \\ \hline
    \textsf{supports} ($n=12$) & \textbf{10} & 0 & 2 \\ \hline
    \textsf{contradicts} ($n=3$) & 0 & \textbf{3} & 0 \\ \hline
  \end{tabularx}
\end{table}

\textbf{Before the retrieval fix.} The $86.7\%$ figure above describes the system \emph{after} the retrieval-side tightening that cut its cited output from $226$ pairs to $24$. The same $200$-pair sample was also scored against the system's \emph{pre-fix} cited output --- i.e.\ before that tightening restricted which pairs were even eligible to be asserted \textsf{supports}/\textsf{contradicts}. Overall precision across all $200$ labelled pairs was $26.5\%$ ($53/200$) for Thái Anh Tuấn and $35.0\%$ ($70/200$) for Đỗ Kim Ngọc; broken down by the system's pre-fix asserted relation, precision on \textsf{contradicts}-labelled pairs was markedly lower than on \textsf{supports} for both annotators --- $7.0\%$ ($8/114$) versus $52.3\%$ ($45/86$) for Thái Anh Tuấn, $8.8\%$ ($10/114$) versus $69.8\%$ ($60/86$) for Đỗ Kim Ngọc. Because the fix only removes pairs and adds none, the same $200$-pair session also yields a valid post-fix estimate restricted to pairs both annotators judged as concerning the correct company: $55.8\%$ ($53/95$) and $56.8\%$ ($21/37$). Read together, precision roughly doubled once the retrieval fix removed off-target evidence, without any change to the annotation instrument, the annotators, or the sample --- the project's strongest externally-validated result. It remains a before/after of the \emph{retrieval} fix specifically: it says nothing about the stage's current $43$-pair output, which is a later and separate change (see above) that has not been annotated at all.

\subsection{Inter-annotator agreement}

Precision figures from a single annotator carry no information about whether the task is well posed. Agreement between two independent annotators does. On these $15$ pairs the two experts assigned identical labels on all $15$.

Perfect agreement makes the usual chance-corrected coefficients uninformative rather than impressive: Gwet's $\mathrm{AC1}$ \cite{gwet2008computing} and Krippendorff's $\alpha$ \cite{krippendorff2018content} both take the value $1$ by construction when observed disagreement is zero, and neither conveys more than the raw count at $n=15$. Raw agreement is therefore reported as the agreement statistic, with the sample size stated beside it rather than a coefficient implying more precision than $15$ items can carry. What the result supports is narrow but real: on the pairs this system actually cites, two senior practitioners working blind and independently do not disagree about what the evidence shows, so the residual shortfall in precision is a property of the system rather than of an ambiguous question.

\subsection{What this measurement can and cannot support}

Five limits belong here rather than in a closing limitations section, because a reader who takes the headline figure without them will overstate it.

\begin{itemize}
  \item \textbf{The figure rests on fifteen pairs.} Its Wilson interval spans $62.1$ to $96.3$ percentage points. The defensible claim is that most of what the system cites is confirmed by a domain expert, not that precision is $86.7\%$.
  \item \textbf{Nine of the $24$ cited pairs carry no expert label.} No figure is claimed for them, and a precision estimate over the system's complete cited output requires annotating them.
  \item \textbf{The labelled pairs are concentrated.} All fifteen belong to a single issuer, ACG, and span eleven distinct claims. The measurement is therefore about one issuer's dossiers, and generalising it across issuers is not supported by this sample.
  \item \textbf{Precision is measured on a deliberately narrow output.} The stage cited $24$ pairs across $464$ claims (\S\ref{sec:experiment-crosscheck}) at the time this annotation was run; high precision on that output is a statement about what the system chose to show, not about how much of the available evidence it finds.
  \item \textbf{The system's currently cited output is different again, and unmeasured.} A 2026-08-13 fix to an adjudicator cache-key defect changed the stage's cited output from $24$ pairs to $43$. No figure in this section --- the $86.7\%$ headline, the pre-fix $26.5\%$/$35.0\%$ numbers, or the perfect agreement statistic --- describes that $43$-pair output; it has not been re-annotated, and none of the above should be read as a measurement of it.
\end{itemize}
