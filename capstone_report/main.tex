...
% Standard academic paragraph spacing: rely on the first-line indent
% (indentfirst, above) to mark a new paragraph, with no extra vertical
% gap on top of it -- LaTeX's own default, matching the reference thesis's
% presentation style. (Previously \parskip was set to a full \baselineskip,
% which stacked a whole blank line on top of the indent and made paragraph
% breaks read as far apart -- supervisor feedback.)
...
% No gap between a heading number and its title at any level (e.g.
% "1.Project Introduction", "1.1.Problem and Motivation") rather than
% KOMA's default wide separator.
...
% Supervisor feedback: figure/table numbers should not carry the chapter
% number (no "Table 3.1", "Figure 2.4") -- just a plain running count
% ("Table 1", "Figure 1", ...) across the whole document, with no reset
...
Greenwashing is a gap between what a company says about its environmental and
social practices and how it behaves, yet detecting it requires evidence a
company's own reports cannot supply. Existing detection systems read only
those reports, carry no explicit notion of time, and are built for
English-language, Western disclosure rather than the Vietnamese market. This
thesis closes that gap by constructing a temporal ESG knowledge graph for
Vietnamese real-estate, construction and building-materials issuers, holding
two evidence channels together --- claims extracted from annual reports and
conduct evidence extracted from independent news --- and adjudicating each
claim against its retrieved conduct evidence as supported, contradicted, or
unverified; no single greenwashing score is produced, since no ground-truth
label exists to validate one against. The approach was evaluated on a
sector-wide corpus of 1,416 annual-report filings from 115 listed issuers
(1,216 parsed and ESG-classified, 873,756 sentences) and 3,797 news articles
from 716 Vietnamese outlets, then instantiated end to end on five issuers
into a resolved graph of 10,624 nodes and 15,130 edges. Cross-checking those
issuers' 464 claims leaves 448 (96.55\%) unverified, a direct consequence of
conduct evidence roughly sixteen times smaller than claim evidence, reported
as an explicit abstention rather than a forced verdict; only 16 claims
receive a verdict, nearly all from a single issuer, An Cường Wood JSC (ticker
ACG), and two external domain experts working blind confirm 62.8\% and
65.1\% of the 43 cited evidence pairs. Retrieval in the evaluated system is
token overlap within a temporal window rather than the graph-structured join
the design also specifies, so this result is a proof of concept concentrated
in one issuer; scaling to the full corpus is engineering already specified
by the pipeline, not an open research question.

\vspace{0.6em}
\noindent\textbf{Keywords:} greenwashing detection; temporal knowledge graph;
ESG disclosure; claim verification; label-free evaluation.
...
\end{longtable}
% The longtable environment above silently steps the `table` counter once
% even though it carries no \caption (no number is ever printed for it) --
% cancel that phantom increment so the first real table below is numbered
% 1, not 2.
...
\subsection{Knowledge-graph-augmented greenwashing detection}
...
\begin{enumerate}
    ...
    \item A label-free evaluation instrument, none of whose checks need a ground-truth label to run, demonstrated on three controlled before/after ablations: for instance, the negative control's same-company evidence-attribution check (Section~\ref{sec:experiment-internal}) moved from 28.76\% (failing) to 100\% (passing, zero cross-feed citations) once the adjudication-cache defect of Section~\ref{sec:crosscheck} was fixed, showing the instrument can detect a real regression and its repair without any label existing for the underlying task.
...
A third pipeline runs on its own schedule and does not depend on the first two. A pair of run-once paths turns published regulatory and GRI text into the controlled vocabulary: a 35-indicator KPI vocabulary and a 136-code GRI catalogue (Figure~\ref{fig:reference-corpus}, Section~\ref{sec:reference-corpus}), consumed downstream by KPI extraction and the indicator axis. The traceability guarantee that runs through all three pipelines (each record keyed by source pdf, page and sentence index, Section~\ref{sec:record-schema}) holds throughout and is not re-derived per stage.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    model/.style ={rectangle, draw=pipmodelline, fill=pipmodelfill, rounded corners=3pt,
                   ...
                   font=\sffamily\small, line width=1.6pt},
    final/.style ={rectangle, draw=pipcontrib, fill=pipoutfill, rounded corners=2pt,
                   ...
                   font=\sffamily\small, line width=2.0pt},
    inter/.style ={rectangle, draw=pipmuteline, fill=pipmutefill, rounded corners=2pt,
                   ...
                   font=\sffamily\scriptsize, line width=0.5pt, text=pipinksoft},
    tag/.style   ={rectangle, draw=pipframeline, fill=pipdpanelfill, rounded corners=2pt,
                   ...
                   font=\sffamily\footnotesize\itshape, text=pipinksoft, line width=0.6pt},
    flow/.style={-{Latex[length=2.2mm, width=1.6mm]}, color=pipinksoft, line width=0.8pt},
    flowd/.style={flow, dashed},
    lbl/.style={font=\sffamily\footnotesize, color=pipinksoft},
    lblt/.style={font=\sffamily\footnotesize\itshape, color=pipinksoft},
    lane/.style={font=\sffamily\bfseries\normalsize, color=pipink},
  ]
  ...
  \node[anchor=north west, font=\sffamily\bfseries\small, color=pipsourceline] at (-1.95,3.40) {REPORT CHANNEL};
  \node[anchor=north west, font=\sffamily\bfseries\small, color=pipsourceline] at (-1.95,-1.32) {NEWS CHANNEL};

  \node[source] (A0) at (0,2.3)
    {\textcolor{pipsourceline}{\faIcon{file-alt}}~\textbf{Annual Reports}\\[1pt]\footnotesize PDF filings};
  ...
  \node[model]  (CLS) at (12.6,0)
    {\textcolor{pipmodelline}{\faRobot}~\textbf{\Large ESG Classifier}\\[3pt]\small ViDeBERTa-v3 \textit{(pretrained model)}\\[2pt]\footnotesize multi-label: E / S / G / Neutral};
  ...
  \node[source] (B0) at (0,-2.3)
    {\textcolor{pipsourceline}{\faNewspaper}~\textbf{News Articles}\\[1pt]\footnotesize search engines \& RSS};
...
ranking bias; Section~\ref{sec:eda-conduct} reports how unevenly the three
...
for the standards corpus (Section~\ref{sec:reference-corpus}), because what this
...
are extracted separately by the KPI-extraction stage (Section~\ref{sec:extraction})
...
scope decision with a consequence stated in Section~\ref{sec:eda-labels} and
revisited in Section~\ref{sec:discussion-threats}: the checkpoint was fitted to
...
and do disagree in both directions, a gap Section~\ref{sec:eda-labels} measures.
...
Section~\ref{sec:eda-sentence}, possible at all.
...
Section~\ref{sec:eda-asymmetry}, would be confounded by the difference between
the annotators.
...
year. Section~\ref{sec:eda-conduct} shows why this matters: a measurable
...
later in this thesis (Section~\ref{sec:experiment-yield}), not by an ablation.
...
negative-control check reported later in this thesis (Section~\ref{sec:experiment-internal})
...
resolved graph, is a result and not an input (Section~\ref{sec:not-dataset});
...
extraction choice discussed in Section~\ref{sec:cleaning}.
...
right, which only the evaluation table requested in Section~\ref{sec:annotation}
can answer.
...
Section~\ref{sec:annotation} explains that a pillar tag needs a sigmoid score of
...
(Section~\ref{sec:extraction}), while the evidence interface's pillar columns are
...
side, as Section~\ref{sec:eda-asymmetry} shows, is environmental-heavy, so the two
...
labelled evaluation set requested in Section~\ref{sec:annotation}. Until then, the
...
Section~\ref{sec:extraction} describes the response: the extraction stage never
...
evaluation set of Section~\ref{sec:annotation}. The claim made here is restricted to what
...
Section~\ref{sec:eda-asymmetry}, is the direct cause of the asymmetric
...
contract of Section~\ref{sec:conduct-channel} exists precisely to carry this
...
\begin{enumerate}
  \item The conduct channel is roughly 15.8$\times$ smaller than the
    ...
    (Section~\ref{sec:eda-asymmetry}). The system therefore has to be able to return
  ...
  \item The conduct channel is concentrated in the last two years while
    claims span fifteen (Section~\ref{sec:eda-conduct}). The retrieval
  ...
  \item The ESG classifier's 34.8\% positive rate is an upper bound,
    with a demonstrated lexical failure mode (Section~\ref{sec:eda-labels}).
    ...
    its own right (Section~\ref{sec:extraction}).
...
Every node's class and every edge's predicate is checked against the schema a second time during ingestion, which is cheap insurance against a regression upstream rather than a repeat of validation already done; on the corpus this chapter reports on, that check raised no warnings. Because every write is a \texttt{MERGE} keyed on \texttt{\_node\_key} or \texttt{\_edge\_key}, re-running the loader against an unchanged \texttt{resolved\_graph.json} changes nothing. This Base Graph is what the next stage queries: Section~\ref{sec:crosscheck} retrieves, for every \texttt{SustainabilityClaim} this graph now holds, the conduct-side evidence sharing its resolved issuer and temporally compatible with it, and adjudicates each pair the way Section~\ref{sec:problem-definition} already defined.

\section{Claim--Conduct Cross-Check}
\label{sec:crosscheck}

This section is where the formal adjudication $f: C \times E \rightarrow \{\textsf{supports}, \textsf{contradicts}, \textsf{irrelevant}\}$ and the claim-level aggregate $g(c)$ defined in Section~\ref{sec:problem-definition} stop being notation and become code, the one stage where the claim side and conduct side are actually brought into contact. Consistent with this thesis's decision never to emit a greenwashing score (Section~\ref{sec:contribution}), the stage writes only these three categorical assessments, never a continuous score.
...
Chapter~\ref{ch:methodology} specified what each stage does; this chapter reports what running them actually produced. The boundary fixed in Section~\ref{sec:not-dataset} still holds: the KPI records, the validated triples and the resolved graph are outputs of the system under study, not corpus components, and they are measured here for the first time rather than assumed. No component of the pipeline is trained, so there is no accuracy metric computed against a held-out split; every number below is a yield, coverage or agreement statistic computed on the pipeline's own output, read directly from the stage-level statistics files each offline stage already writes as a side effect of running (Section~\ref{sec:validate-normalize}, Section~\ref{sec:entity-resolution}, Section~\ref{sec:indicator-axis}). Two scopes are reported separately because the pipeline itself runs at two different scopes: Section~\ref{sec:experiment-yield} covers the full sector corpus Extraction and Validate \& Normalize have processed to date; Section~\ref{sec:experiment-crosscheck} covers Claim--Conduct Cross-Check, which is issuer-scoped by construction (Section~\ref{sec:crosscheck}) and has so far been run to completion for five issuers, AAA, ACC, ACG, ADP, AGG, rather than one.
...
\begin{table}[htbp]
  ...
  \caption[Notation used in this chapter]{Notation used throughout this
    ...
    Section~\ref{sec:experiment-expert}.}
...
Table~\ref{tab:validate-yield} shows only one KPI observation in five mapped onto the controlled 35-indicator vocabulary. Read alone, that looks like a low yield, but it is not simply an extraction failure. Canonicalisation tallies a mapping decision once per \emph{distinct} \texttt{KPIObservation} node rather than per occurrence patched (7,324 occurrences map onto the 7,127 distinct nodes in the table, the denominator every percentage below is taken over), and its design rule favors precision over recall: an ambiguous case is rejected rather than force-mapped (Section~\ref{sec:validate-normalize}). The single largest rejection reason, 3,094 of the 5,663 unmapped nodes (54.6\% of all rejections), is exactly the case the rule targets: a value reported in a financial unit (VND, tỷ đồng) the regulatory indicator vocabulary has no legal place for, not a KPI the extractor misread. The other structural driver is unit inconsistency at the source: units are written 189 distinct raw ways before normalisation, an expected consequence of extracting from unedited natural-language text rather than a structured filing, which is why a rejected occurrence stays fully readable instead of being silently overwritten.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|L{0.8}|L{1.2}|}
    ...
    Indicator axis added, this recorded run (Section~\ref{sec:indicator-axis}) & +26 nodes / +2,018 edges \\ \hline
...
\begin{table}[htbp]
  ...
  \caption[Internal quality controls]{Internal quality controls over the
    ...
    count from Table~\ref{tab:resolve-yield} (Section~\ref{sec:experiment-yield}),
...
The final control is the only one built so the system can lose. Rather than checking the system against its own design, it tests the null hypothesis that cited evidence for issuer $T$ is no more likely to come from $T$'s own crawled feed than that feed's overall share of the conduct pool would predict (lift $\approx 1$); a lift of $2$ or more is read as genuine company-specific attribution. On the cited output measured here it passes outright: all $24$ cited pairs draw solely on the examined issuer's own feed, zero cross-feed citations, though that $100\%$ rests on only $24$ items, giving a Wilson lower bound of $86.2\%$, a statement about attribution discipline on a narrow slice of the $464$ claims, not about coverage. (The cited population has since grown to $43$ pairs, Section~\ref{sec:experiment-expert}; the control has not been re-run at that scale.)

This is also the thesis's one controlled before/after. The identical check, run on the $226$-pair population that predated a 2026-08-07 adjudication-cache fix (Section~\ref{sec:crosscheck}), had failed outright at $28.76\%$ ($65/226$), no better than base rate. But the fix bundled more than the cache key: it also issuer-scoped conduct retrieval, added a topic-overlap gate, and tightened the adjudication prompt, and the pre-fix run itself was adjudicated by OpenAI, a provider removed before the fix landed. The comparison is therefore diagnostic, showing that a real regression was caught and repaired, rather than a fully isolated, single-variable ablation.

Together, these controls establish structural, provenance and retrieval sanity, that the graph is well-formed and its citations are genuine, while Section~\ref{sec:experiment-expert} supplies the external, semantic evaluation of whether the verdicts built on top of them are actually correct.
...
The unit of analysis is a (claim, evidence) pair the system cited. Cross-Check writes a pair into a dossier only when the adjudicator judged it \textsf{supports} or \textsf{contradicts}; a pair judged \textsf{irrelevant} is discarded and leaves no trace (Section~\ref{sec:crosscheck}).
...
\begin{itemize}
  ...
  \item Precision is measured on a narrow slice of the system's output. The stage cites $43$ pairs across $464$ claims (Section~\ref{sec:experiment-crosscheck}), so high precision on this output is a statement about what the system chose to show, not about how much of the available evidence it finds; Section~\ref{sec:experiment-expert} above establishes that this instrument cannot measure recall at all.
\end{itemize}
...
Finally, the report-side extraction pipeline is a positional parser, not a layout-aware one, and this remains an argued design choice rather than a measured one. Section~\ref{sec:cleaning} exact-page-anchors every sentence at the cost of table reconstruction; the longest artefact this produces is a single 11,827-character text run that is a flattened table, not a sentence. No ablation in this thesis quantifies how much information, or how much noise, this trade-off costs against the layout-aware parser used for the much smaller reference corpus (Section~\ref{sec:reference-corpus}); the choice is defended qualitatively, not empirically.
...
This thesis tested whether a temporal knowledge graph fed by two structurally independent evidence channels, company reports and external news, can make greenwashing in Vietnamese real-estate, construction and building-materials disclosure checkable instead of merely readable. Greenwashing, in the sense Delmas and Burbano define it, sits at the gap between what a company says and how it behaves, so a system reading only company reports cannot observe that gap regardless of its architecture. The system keeps the two channels distinguishable through a source-type stamp and carries validity time and page-level provenance through every stage, so a claim made in one year can be checked against conduct observed in another and traced back to the page it came from.

Run end to end on five issuers, the pipeline produced a schema-compliant, well-provenanced graph of 10,624 nodes and 15,130 edges, with 100\% schema compliance and 97.78\% round-trip KPI grounding. Two domain experts working independently and blind to the system's verdicts confirmed its output roughly two-thirds of the time, with near-perfect agreement between them. Both results are genuine, but they cover a narrow slice of the total output: only 16 of 464 claims received a verdict, almost all from a single issuer, and retrieval relied on lexical overlap instead of the graph-native indicator-axis join the design also specifies. The remaining gap sits not in the engineering or the schema, which hold up, but in how much independent conduct evidence the system currently has to check claims against.

Closing that gap needs more conduct evidence, not a new model or schema. Three steps are already specified by the existing design and need no new research: activating the graph-native indicator-axis retrieval tier instead of only measuring its coverage; driving the self-verification guard from \texttt{config/issuer\_registry.json} so it covers every issuer instead of one; and scaling ingestion from five issuers to the full 115-issuer, sector-wide corpus already built and classified.

One design choice is worth keeping regardless of scale. Returning \textsf{unverified} as a first-class outcome, instead of forcing every claim to a verdict, suits a conduct channel this thin. Withholding a greenwashing score, given that no ground truth exists for this or any Vietnamese corpus, applies the same caution one level higher, and is what this thesis contributes to a problem usually approached the other way around.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|L{0.761}|L{0.423}|L{1.817}|}
    ...
    \texttt{esg} & bool & Binary ESG relevance, decided by the Neutral score (Section~\ref{sec:annotation}) \\ \hline
