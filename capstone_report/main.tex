...
The ACC organisation node of Table~\ref{tab:entity-resolution-example} --- 74 raw mentions collapsed by the frozen issuer anchor onto one canonical node --- is a real instance of the chain this pass builds, closing the loop that section left open: what does a merge like that become once it is actually loaded? Organization is a \texttt{supersedes}-legal class, so all 74 entries in its \texttt{temporal\_versions} list materialise as 74 version nodes chained beneath the canonical node. The result is not quite what the entity-resolution table alone suggests: the canonical node keeps the issuer registry's legal name and inherits its \texttt{valid\_from} from whichever raw mention is most recent, but that most-recent mention itself --- the trading name ``Becamex ACC'', dated 2024-01-01 --- is written out as its own node one hop below, with \texttt{is\_current} set \texttt{false}. The canonical node is the identity a query addresses; the chain beneath it is the provenance a reader can walk one version at a time.

Every node's class and every edge's predicate is checked against the schema a second time during ingestion --- not because Validate \& Normalize and Entity Resolution did not already validate the graph, but because a warning that costs nothing to print is cheap insurance against a regression upstream; on the corpus this chapter reports on, that check raised no warnings, and \texttt{--strict} exists to turn a future one into an abort rather than a log line. Because every write is a \texttt{MERGE} keyed on \texttt{\_node\_key} or \texttt{\_edge\_key}, re-running the loader against an unchanged \texttt{resolved\_graph.json} changes nothing.
...
\textbf{Adjudication.} Each admitted pair is judged independently by a large language model instructed to read exactly one claim and one piece of conduct evidence and return one of \textsf{supports}, \textsf{contradicts} or \textsf{irrelevant} with a confidence score and a Vietnamese-language rationale --- never translated, so a human reviewer checking the system's reasoning against the original report or article reads the same language throughout. The instruction is deliberately narrow about what counts as a contradiction: a generic adverse event does not, by itself, contradict a claim about an unrelated specific topic, a rule tightened after a real case where a stock-manipulation penalty was read by an earlier prompt as also undermining an unrelated governance claim. \texttt{Adjudicator} keeps its own small provider registry --- \texttt{gemini} (the default), \texttt{deepseek} and \texttt{openai} --- rather than calling the shared kernel factory the rest of the pipeline uses, since this class is stage logic rather than kernel; a single active provider is the intended and normal use. Every reply is checked against the literal set of three verdict strings before anything downstream trusts it, so a syntactically valid but wrongly-shaped reply is rejected rather than misread as a verdict. Adjudications are cached by the exact claim and evidence text sent, not by which nodes produced them, so a boilerplate sentence repeated verbatim across several report years is billed once rather than per occurrence, and a cache entry is written only for a reply actually returned, never for a provider that could not be reached.

\textbf{The self-verification guard.} A \textsf{supports} verdict is not written back to the graph unconditionally. Before it becomes a \texttt{verifiedBy} edge, the evidence's source domain is checked against a curated list of the issuer's own web properties; a match --- \texttt{aaa.com.vn}, \texttt{anphatholdings.vn} and a handful of others for the issuer this chapter reports on --- drops the edge while keeping the evidence record itself visible in the dossier, marked not independent and carrying the reason it was dropped. The guard applies only on the \textsf{supports} branch; there is no equivalent check on \textsf{contradicts}, since a company website corroborating its own claim is exactly the self-verification this thesis's conduct channel exists to rule out (\S\ref{sec:problem-definition}), while a company website contradicting its own claim carries no comparable risk. \textbf{The curated list itself is populated for this issuer only.} Across all five issuers evaluated in Chapter~\ref{ch:experiment}, zero \textsf{supports} edges have ever been dropped by this guard --- every cited item so far comes from a third-party outlet (\texttt{cafef.vn}, \texttt{baodautu.vn}, \texttt{mekongasean.vn} and similar) rather than a company-owned domain, so the guard has not yet been exercised for the four issuers besides the one it was written against, and would not currently fire even if one of them cited its own domain (\S\ref{sec:discussion-threats}).
...
\textbf{A null result, reported honestly.} Run end to end against the issuer worked through above, this stage returns \textsf{unverified\_insufficient\_evidence} for every one of that issuer's 36 claims --- not because the machinery failed, but as the mechanical consequence of the claim/conduct asymmetry \S\ref{sec:eda-asymmetry} already measured. \S\ref{sec:experiment-crosscheck} reports that run in full, alongside the four other issuers the stage has since been run against. What matters here is that a claim can arrive at that one label by more than one road: Table~\ref{tab:crosscheck-two-claims} shows two real claims taking two different, equally informative, ones.
...
Table~\ref{tab:validate-yield} records that only one KPI observation in five is mapped onto the controlled 35-indicator vocabulary; read alone, that looks like a low yield. Two populations sit behind that ratio and are worth separating: canonicalisation patches every one of the 7,324 \texttt{KPIObservation} occurrences it walks, but tallies a mapping decision once per \emph{distinct} node, so the 7,127 in the table --- not the occurrence count --- is the denominator every percentage below it is taken over. It is not a matching failure so much as the matching gate refusing to guess: canonicalisation's own design rule is precision over recall, rejecting an ambiguous case rather than force-mapping it (\S\ref{sec:validate-normalize}), and the single largest rejection reason --- 3,094 of the 5,663 unmapped nodes, 54.6\% of all rejections --- is exactly the case the rule was written for, a value reported in a financial unit (VND, tỷ đồng) that the regulatory indicator vocabulary has no legal place for, not a KPI the extractor misread. The other structural driver is unit inconsistency at the source: units are written 189 distinct raw ways before normalisation (\textit{tỷ đồng} vs \textit{ty dong} vs \textit{VND}, \textit{tấn/năm} vs \textit{tấn}, a bare \textit{\%} vs \textit{percent}) --- an expected consequence of extracting from unedited natural-language report and news text rather than a structured filing, and precisely why canonicalisation writes \texttt{kpi\_id} as a new property alongside the raw one instead of overwriting it (\S\ref{sec:validate-normalize}): a rejected occurrence stays fully readable, just outside the axis this thesis measures against.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|L{0.8}|L{1.2}|}
    ...
    \textbf{Final graph} & \textbf{10,624 nodes / 15,130 edges} \\ \hline
...
As Table~\ref{tab:resolve-yield} shows, Entity Resolution's deterministic pass alone --- identity-key equality plus the two frozen anchors, no embedding or LLM call --- removes 23.0\% of raw nodes; \texttt{--no-llm} is the operating mode recorded throughout this thesis (\S\ref{sec:entity-resolution}), so this reduction is what Stage A and the anchors deliver on their own, with Stage B/C's embedding-and-adjudication pass still dormant on top of it. The indicator axis reaches just over half of every claim-like node (\texttt{SustainabilityClaim}, \texttt{Goal}, \texttt{Initiative} together) and 62.2\% of \texttt{SustainabilityClaim} specifically --- the narrower, higher figure is expected, since a \texttt{SustainabilityClaim} is extracted from report prose that more often names a concrete, indicator-shaped topic than a \texttt{Goal} or \texttt{Initiative} does. This coverage is a static structural property of the resolved graph, available in principle to a graph-native join; whether the retrieval-side indicator-axis tier in \S\ref{sec:experiment-crosscheck} actually draws on it in a given run is a separate, empirical question, taken up there rather than assumed here.

\textbf{The 649 and 718 above are not the same population, and do not have to reconcile 1:1.} \texttt{alignsWithIndicator} edges accumulate across runs of this append-only pass rather than being rebuilt from a clean graph each time (\S\ref{sec:entity-resolution}): 649 is the number this specific recorded run \emph{created}, read from its own stage statistics, while 718 --- and the resolved graph's 807 \texttt{alignsWithIndicator} edges on disk that back it --- is the \emph{cumulative} count on the graph as loaded, which includes edges written by earlier runs still present in the file. Both figures are individually faithful to their source; a reader should not expect 718 nodes to be reachable from only 649 edges, since the two are drawn from different points in the graph's accumulation history, not from one snapshot.
...
\begin{table}[htbp]
  ...
  \caption[Internal quality controls]{Internal quality controls over the
    ...
    was run. $M_{4.1}$'s numerator (718) is a cumulative count against the
    append-only \texttt{alignsWithIndicator} edge set, not the 649 edges
    Table~\ref{tab:resolve-yield} records this run as having created ---
...
\]
A lift near $1$ means the null cannot be rejected, and no downstream conclusion may then be read as specific to the company named in it; a lift of $2$ or more is taken as genuine company signal. On the cited output measured here the control passes outright: all $24$ cited pairs draw on the examined issuer's own feed, with zero cross-feed citations. That $100\%$ rests on $24$ items, so its Wilson lower bound is $86.2\%$, not $100\%$ --- and it is bought by citing narrowly: $24$ pairs across $464$ claims. It is a statement about attribution discipline, not about coverage. (The cited population has since grown to $43$ pairs under the same fix, \S\ref{sec:experiment-expert}; the negative control has not been re-run at that scale, so $86.2\%$ remains the reported lower bound rather than a recomputed $43$-pair figure.)

\textbf{This same-company control is also the one controlled before/after this thesis reports.} Before the adjudication-cache defect of \S\ref{sec:crosscheck} was fixed --- the cache key omitted the prompt, so a re-run silently replayed pre-fix verdicts rather than recomputing them --- the identical check, run against the $226$-pair population that predated the fix, failed outright: $28.76\%$ same-company attribution ($65/226$), no better than the corpus-wide base rate. After the fix, the same check on the fix's immediate output passed outright, as reported above. Both runs use the identical statistic and threshold; only the system state changed between them, and the same-company evidence rate is exactly the property the fix targeted, so this is a genuine before/after rather than an incidental change. It is also, on inspection, the only one of the three ablations \S\ref{sec:contribution} originally claimed that was ever actually run: no time-window or layout-parser ablation exists in this thesis (\S\ref{sec:cleaning}, \S\ref{sec:reference-corpus}), and the indicator-axis join's contribution is reported as a yield (\S\ref{sec:experiment-yield}), not as a before/after comparison.
