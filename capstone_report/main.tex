...
the 24 (claim, evidence) pairs the stage cites, 15 carry blind labels from two
external domain experts, who confirm 13 of them (86.7\%, [62.1, 96.3]) and
assign identical labels on all 15; those 15 concern one issuer, so the figure
describes precision on a narrow and deliberately conservative output rather
than on the system at large. Scaling the graph beyond five issuers is
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|L{0.42}|L{1.58}|}
    ...
    $S \subseteq P$ & the cited pairs carrying expert labels, attention-check items excluded \\ \hline
...
The unit of analysis is a (claim, evidence) pair the system cited. Cross-Check writes a pair into a dossier only when the adjudicator judged it \textsf{supports} or \textsf{contradicts}; a pair judged \textsf{irrelevant} is discarded and leaves no trace (\S\ref{sec:crosscheck}). Across the five issuers the stage cites $24$ pairs in total, and $15$ of those carry blind expert labels. Those $15$ are the population measured here. The remaining $9$ are unlabelled, and no figure in this thesis is claimed for them.
...
Attention checks are a validity gate, not a result. Real claims paired with unrelated evidence --- near-certainly \textsf{irrelevant} --- were shuffled into each annotator's sheet, and the protocol voids a session in which more than $3$ of the $20$ are marked as evidence. Both annotators marked exactly $2$, so both sessions stand, and the two items they marked were the same two, which suggests those pairs happen to be genuinely related rather than that either annotator was inattentive.
...
\begin{itemize}
  ...
  \item \textbf{Precision is measured on a deliberately narrow output.} The stage cites $24$ pairs across $464$ claims (\S\ref{sec:experiment-crosscheck}); high precision on that output is a statement about what the system chooses to show, not about how much of the available evidence it finds.
\end{itemize}
