...
Run AFTER step05 and BEFORE step06 (re-run it after any step05 re-run), from the
REPO ROOT:

    python src_module/run.py provenance --dry-run   # preview coverage
    python src_module/run.py provenance             # stamp resolved_graph.json in place

MIGRATED FROM ``src/step05b_stamp_provenance.py`` (2026-07-27), verbatim: this file is
that one with the docstring and the import block replaced, and NOTHING else. It is the
first stage to move without extracting a new ``core/`` module -- the step03b slice had
already lifted all four symbols it needs (``PROVENANCE_CLASSES``,
``get_stable_entity_id``, ``parse_source_id`` -> ``core.identity``; ``get_identity_keys``
-> ``core.schema``). No logic line differs. Model A: the ``src/`` original keeps running
untouched and ``test/test_esg_kg_provenance.py`` holds the two trees equal on the real
10k-node graph.
"""
...
from esg_kg.core.schema import get_identity_keys
