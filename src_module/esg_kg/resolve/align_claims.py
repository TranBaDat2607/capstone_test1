...
Run from the repo root, after step05c and before step06:
  python src_module/run.py align_claims --dry-run
  python src_module/run.py align_claims --max-llm-pairs 200
Equivalently, from inside src_module/:  python -m esg_kg.resolve.align_claims --dry-run

--------------------------------------------------------------------------------------
Moved verbatim from src/step05d_align_claims_to_indicators.py (Model A: that file still
exists and still runs). NO logic line changed. What differs:

  * the docstring and the import block — the five symbols this stage used to take from
    sibling STAGES all come from the kernel now: `REPO_ROOT` <- core.paths,
    `load_schema_sets` <- core.schema, `GraphPatch`/`temporal_md` <- core.graph_patch (the
    05c slice lifted them precisely because THIS file imported them from a stage),
    `_OpenAIProvider` <- core.llm. Third stage after 05b and 03 to move without extracting
    any new core/ module.
...
one takes a PROPERTIES DICT, step07's takes a NODE and dispatches on its class. Unifying
them would silently rewrite step07's paid adjudication prompt. Keep them separate — the
migration of step07 must move its own copy (PIPELINE.md §2.1).

`SYSTEM` and `build_prompt` are paid behaviour, not prose. Rewording either still "works"
at runtime while changing every classification the stage has ever made, so
`test/test_esg_kg_align_claims.py` pins them byte-for-byte.

THE IN-PLACE-PATCH QUESTION (PIPELINE.md §3): meeting its own past output this stage SKIPS
— `candidates` excludes any node that already carries an `alignsWithIndicator` edge, from
either tier. That is 05c/03b's shape, so an arm run against an already-aligned artifact
would go vacuous. It does not today only because no `alignment_method=llm` edge exists in
the live snapshot; the test strips them defensively regardless.
"""
