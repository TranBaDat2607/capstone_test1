...
stage) passes `adjudication_cache=None`, so `python src_module/run.py entities` behaves
...
Run from the repo root:
    python src_module/run.py entities --dry-run
    python src_module/run.py entities
Equivalently, from inside src_module/:  python -m esg_kg.resolve.entities --dry-run
"""
...
def resolve_graph(triples: List[Dict[str, Any]], idkeys: Dict[str, List[str]], *,
    ...
    if not no_llm and client is not None:
        ...
        for cls, roots in class_roots.items():
            ...
            embs = embed_texts(texts, client, embed_model, embed_dim, rate_limiter, embed_batch)
            sims = embs @ embs.T
