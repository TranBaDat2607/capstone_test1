...
Run from the repo root (Neo4j from step 5 must be up):
  python src_module/run.py neo4j_sync --dry-run          # counts only, no write
  python src_module/run.py neo4j_sync                    # MERGE into Neo4j
  python src_module/run.py neo4j_sync --clear-advisory   # wipe prior advisory layer first
...
def run(args: argparse.Namespace) -> None:
    ...
    logger.info("Sync complete. The ledger + Cypher can now read everything from Neo4j "
                "(python src_module/run.py claim_ledger).")
