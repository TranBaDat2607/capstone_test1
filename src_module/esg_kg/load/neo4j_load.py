...
Run from the repo root:  python src_module/run.py neo4j_load
Connection defaults target a dedicated Neo4j Community instance; override via .env
(NEO4J_URI / NEO4J_USER / NEO4J_PASSWORD) or CLI flags.

MIGRATED FROM ``src/step06_load_graph_to_neo4j.py`` (2026-07-29), verbatim except the
docstring and the import block: ``REPO_ROOT`` now comes from ``esg_kg.core.paths`` (the
same swap every leaf stage has made since step01) and ``load_schema_sets`` now comes from
``esg_kg.core.schema`` (lifted from step03 on 2026-07-28). No logic line differs. This is
the second Neo4j-writing stage to migrate, after ``esg_kg.load.neo4j_sync`` (step08); unlike
step08 it also drives writes through ``session.execute_write(lambda tx: tx.run(...)
.consume())`` and reads results back in ``print_graph_stats``, so
``test/test_esg_kg_neo4j_load.py`` stubs the installed ``neo4j`` package's
``GraphDatabase`` attribute with a fake that supports both the bare-``session.run()`` and
``execute_write``/``tx.run()`` call shapes, plus a read-back ``.single()``/iteration shape,
still recording every Cypher string + parameter dict without executing anything for real.
"""
