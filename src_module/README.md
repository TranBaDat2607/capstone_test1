...
| Stage kế tiếp | 🟢 `03`, `05d`, `07`, `04` đã dời, còn **bốn** stage đủ điều kiện: `01`, `05`, `06`, `09` — cộng **`08`**/**`10`**, hai stage `07` đã mở khoá (chờ `node_text`/`Adjudicator` từ chính stage đó, không còn chờ kernel). `01` là hub cuối cùng thật sự còn lại (bị import phần *stage-local*, không phải phần đã lên kernel) — theo DESIGN.md §4 nó vẫn để cuối dù không còn gì chặn kỹ thuật. `step05` **chưa được dời** cho tới khi xử §3.1 — nay mặc định là gộp khối theo §3.2. PIPELINE.md §2.1 |
| `step07b` (softmax) | ⛔ **không dời** — UI `frontend/`+`api/` không đọc; giữ chạy ở `src/` (DESIGN.md §4.1) |

`src/` **vẫn là pipeline chạy thật**; mới **chín** stage chạy được từ đây (`00`, `03`, `03b`,
`03c`, `04`, `05b`, `05c`, `05d`, `07`) **cộng một khối** `build_validated` — `run.py --list` là
nguồn sự thật — và bản `src/step00_graph_quality_report.py` vẫn còn (nợ đã ghi: DESIGN.md §6.1).
