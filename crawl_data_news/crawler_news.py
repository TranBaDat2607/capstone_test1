"""
FPT News Crawler v5 — Thu thập tin tức FPT từ báo chính thống Việt Nam.

Cải tiến v5:
  - Playwright dùng domcontentloaded (thay networkidle) → nhanh gấp 10x
  - Thêm Google Search meta-engine để tìm bài cũ 2009-2019
  - Tối ưu tốc độ: giảm delay, tăng MAX_ARTICLES_PER_SOURCE
  - Thanh Niên: dùng requests thay Playwright extract (nhanh hơn rất nhiều)
"""
...
SEARCH_KEYWORDS = ["FPT", "FPT Software", "FPT Telecom", "FPT IS"]
...
def is_fpt_related(title: str, content: str) -> bool:
    ...
    return any(m in text for m in [
        "fpt", "fpt software", "fpt telecom", "fpt education",
        "fpt semiconductor", "fpt is", "fpt digital", "fpt smart cloud",
        "fpt play", "fpt shop", "tập đoàn fpt", "trương gia bình",
    ])
...
def extract_year_from_url(url: str) -> str | None:
    # Tuổi Trẻ: ...20250307125703695.htm
    m = re.search(r"-(\d{4})(\d{2})(\d{2})\d{6,}\.htm", url)
    if m:
        y = int(m.group(1))
        if 2005 <= y <= 2030:
            return str(y)
    # Thanh Niên: ...185250310...htm
...
def build_article(url: str, html: str) -> dict | None:
...
def fetch_and_build(url: str) -> dict | None:
    resp = safe_get(url)
    if not resp:
        return None
...
def search_vnexpress(keyword: str, year: int = None) -> list[str]:
    urls = []
    for pg in range(1, MAX_PAGES_PER_KEYWORD + 1):
        ...
        r = safe_get(f"https://timkiem.vnexpress.net/?{params}")
        if not r:
            break
...
def search_tuoitre(keyword: str) -> list[str]:
    urls = []
    for pg in range(1, MAX_PAGES_PER_KEYWORD + 1):
        r = safe_get(f"https://tuoitre.vn/tim-kiem.htm?keywords={quote(keyword)}&page={pg}")
        if not r:
            break
        soup = BeautifulSoup(r.text, "lxml")
        found = 0
...
def search_vietnamnet(keyword: str) -> list[str]:
    urls = []
    for pg in range(1, 6):
        r = safe_get(f"https://vietnamnet.vn/tim-kiem?q={quote(keyword)}&page={pg}")
        if not r:
            break
        soup = BeautifulSoup(r.text, "lxml")
        found = 0
...
def crawl_with_requests(source_key: str, name: str, search_fn,
                        use_year: bool, use_google_old: bool = False) -> dict:
    """Crawl nguồn báo dùng requests (không cần Playwright extract)."""
    logger.info(f"\n{'='*60}")
    logger.info(f"  📰 {name}")
    logger.info(f"{'='*60}")
    ...
    if use_year:
        for year in CRAWL_YEARS:
            for kw in SEARCH_KEYWORDS:
                ...
                found = search_fn(kw, year=year)
                logger.info(f"     → {len(found)} URL")
        ...
        if domain:
            for year in range(2010, 2020):
                for kw in ["FPT", "FPT Software"]:
                    ...
                    found = search_google_site(domain, kw, year)
                    before = len(all_urls)
                    all_urls.update(found)
                    logger.info(f"     → +{len(all_urls) - before} URL mới")
                    time.sleep(2.0)  # Google rate limit
    ...
    for i, url in enumerate(url_list, 1):
        logger.info(f"  [{i}/{len(url_list)}] {url[:80]}...")
        article = fetch_and_build(url)
        if not article:
            stats["failed"] += 1
            time.sleep(REQUEST_DELAY)
            continue
        ...
        logger.info(f"    ✓ {year}/{fp.name} | {article['title'][:40]} | {article['published_date']}")
        stats["success"] += 1
...
def crawl_thanhnien() -> dict:
    """Crawl Thanh Niên: Playwright search + requests extract (nhanh!)."""
    logger.info(f"\n{'='*60}")
    ...
    # 1. Search URLs bằng Playwright (nhanh vì dùng domcontentloaded)
    all_urls = set()
    try:
        ...
        with sync_playwright() as p:
            ...
            for year in range(2010, 2020):
                for kw in ["FPT", "FPT Software"]:
                    ...
                    found = search_google_site("thanhnien.vn", kw, year)
                    before = len(all_urls)
                    all_urls.update(found)
                    logger.info(f"     → +{len(all_urls) - before} URL mới")
                    time.sleep(2.0)
    ...
    # 2. Extract bằng requests (KHÔNG dùng Playwright) → nhanh hơn rất nhiều
    for i, url in enumerate(url_list, 1):
        logger.info(f"  [{i}/{len(url_list)}] {url[:80]}...")
        article = fetch_and_build(url)
        if not article:
            ...
            continue
        year = get_year(article["published_date"], url)
        fp = save_article(article, year, "thanhnien")
        logger.info(f"    ✓ {year}/{fp.name} | {article['title'][:40]} | {article['published_date']}")
        stats["success"] += 1
        stats["by_year"][year] = stats["by_year"].get(year, 0) + 1
        time.sleep(REQUEST_DELAY)
...
def main():
    logger.info("=" * 60)
    logger.info("  FPT NEWS CRAWLER v5")
    ...
    try:
        s = crawl_with_requests("vnexpress", "VnExpress", search_vnexpress,
                                use_year=True, use_google_old=True)
    ...
    try:
        s = crawl_with_requests("tuoitre", "Tuổi Trẻ", search_tuoitre,
                                use_year=False, use_google_old=True)
    ...
    # Thanh Niên — Playwright search + requests extract + Google bổ sung
    try:
        s = crawl_thanhnien()
    ...
    try:
        s = crawl_with_requests("vietnamnet", "VietnamNet", search_vietnamnet,
                                use_year=False, use_google_old=True)
    ...
    for sk, st in total_stats["sources"].items():
        if "error" in st:
            ...
            logger.info(f"  {sk}: ✓{st['success']} ✗{st['failed']}")
